...
public class PlayerRecViewAdapter2 extends RecyclerView.Adapter<PlayerRecViewAdapter2.ViewHolder> implements Filterable {

    private ArrayList<Player> players = new ArrayList<>();
    private Context context;
    private List<Player> allPlayers;

    public PlayerRecViewAdapter2(Context context) {
        this.context = context;
    }

    @NonNull
    @Override
    public PlayerRecViewAdapter2.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_item_players,parent,false);
        return new PlayerRecViewAdapter2.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull  PlayerRecViewAdapter2.ViewHolder holder, int position) {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint.toString().isEmpty()){
                ...
                for (Player play: allPlayers){
                    if (play.getName().toLowerCase().contains(constraint.toString().toLowerCase())) {
